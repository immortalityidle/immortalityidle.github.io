...
export class InventoryService {
  ...
  generateWeapon(
    ...
    this.logService.addLogMessage(
      'Your hard work paid off! You created a new weapon: ' + name + '!',
      'STANDARD',
      'CRAFTING'
    );
  ...
  generatePotion(grade: number, masterLevel: boolean): void {
    ...
    this.logService.addLogMessage(
      'Alchemy Success! Created a ' + name + '. Keep up the good work.',
      'STANDARD',
      'CRAFTING'
    );
  ...
  generateArmor(
    ...
    this.logService.addLogMessage(
      'Your hard work paid off! You created some armor: ' + name + '!',
      'STANDARD',
      'CRAFTING'
    );
  ...
  addItem(item: Item, quantity = 1): number {
    ...
    if (this.autoSellUnlocked && !this.hellService?.inHell) {
      this.logService.addLogMessage(
        `You don't have enough room for the ${item.name} so you sold it.`,
        'STANDARD',
        'EVENT'
      );
      this.characterService.characterState.money += item.value * quantity;
  ...
  equip(itemStack: ItemStack): void {
    ...
    if ((item.armorStats?.durability || 0) <= 0 && (item.weaponStats?.durability || 0) <= 0) {
      ...
      this.logService.addLogMessage(
        'You tried to equip some broken equipment, but it was broken.',
        'STANDARD',
        'EVENT'
      );
      return;
