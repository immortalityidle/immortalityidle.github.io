    ...
    consequence: [
      () => {
        if (this.characterService.characterState.attributes.charisma.value < 1e6) {
          this.logService.addLogMessage(
            'You completely fail to catch the attention of any of the damned.',
            'STANDARD',
            'EVENT'
          );
          return;
        ...
        if (Math.random() < 0.01) {
          ...
          this.logService.addLogMessage(
            'Your recruiting efforts seem to infuriate the demons here.',
            'STANDARD',
            'EVENT'
          );
        } else {
    ...
    consequence: [
      () => {
        if (this.characterService.characterState.attributes.strength.value < 1e24) {
          this.logService.addLogMessage(
            "You try to crack into the pillar, but you're not strong enough.",
            'STANDARD',
            'EVENT'
          );
          return;
    ...
    consequence: [
      () => {
        if (this.characterService.characterState.attributes.fireLore.value < 1e16) {
          this.logService.addLogMessage(
            "Your connection to fire isn't nearly as strong as you thought it was.",
            'STANDARD',
            'EVENT'
          );
          return;
    ...
    consequence: [
      () => {
        if (this.characterService.characterState.attributes.waterLore.value < 1e16) {
          this.logService.addLogMessage(
            "Your connection to water isn't nearly as strong as you thought it was.",
            'STANDARD',
            'EVENT'
          );
          return;
    ...
    consequence: [
      () => {
        ...
        if (this.characterService.characterState.attributes.charisma.value <= 1e24) {
          this.logService.addLogMessage(
            'The damned souls completely ignore your attempts at instruction.',
            'STANDARD',
            'EVENT'
          );
          return;
    ...
    consequence: [
      () => {
        ...
        if (Math.random() < threshold) {
          ...
          this.logService.addLogMessage(
            'You very nearly make out the meaning of the scrawled contract.',
            'STANDARD',
            'EVENT'
          );
        }
  ...
  hells: Hell[] = [
    ...
    },
    {
      ...
      dailyEffect: () => {
        ...
        this.logService.addLogMessage(
          'The knives dig into your flesh, causing ' + damage + ' damage.',
          'INJURY',
          'COMBAT'
        );
        this.characterService.characterState.status.health.value -= damage;
    ...
    },
    {
      ...
      dailyEffect: () => {
        ...
        if (this.inventoryService.consume('iceCore') < 0) {
          ...
          this.logService.addLogMessage(
            'The steam cooks your skin, causing ' + damage + ' damage.',
            'INJURY',
            'COMBAT'
          );
          this.characterService.characterState.status.health.value -= damage;
    ...
    },
    {
      ...
      dailyEffect: () => {
        ...
        if (this.inventoryService.consume('iceCore') < 0) {
          ...
          this.characterService.characterState.status.health.value -= damage;
          this.logService.addLogMessage(
            'The heat of the pillars burns you for ' + damage + ' damage.',
            'INJURY',
            'COMBAT'
          );
        }
    ...
    },
    {
      ...
      dailyEffect: () => {
        ...
        if (damage > 0) {
          this.logService.addLogMessage(
            "The mountain's blades shred you for " + damage + ' damage.',
            'INJURY',
            'COMBAT'
          );
        }
    ...
    },
    {
      ...
      dailyEffect: () => {
        ...
        if (this.inventoryService.consume('fireCore') < 0) {
          this.characterService.characterState.status.health.value -= damage;
          this.logService.addLogMessage(
            "The mountain's ice freezes you for " + damage + ' damage.',
            'INJURY',
            'COMBAT'
          );
        }
